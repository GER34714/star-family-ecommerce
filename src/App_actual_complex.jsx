...
export default function StarFamilyApp() {
  ...
  // CAMBIO 1: Estado para categorías dinámicas desde Supabase
  ...
  // CAMBIO 4: Estado para zonas de envío
  const [selectedZone, setSelectedZone] = useState(null);
  const [shippingCutoffHour, setShippingCutoffHour] = useState(() => {
    const saved = getStorageItem("shipping_cutoff_hour");
    return saved !== null ? saved : 12;
  });
  
  // CAMBIO 8: Estado para paginación
  const [currentPage, setCurrentPage] = useState(1);
  const [productsPerPage, setProductsPerPage] = useState(() => {
    const saved = getStorageItem("products_per_page");
    return saved !== null ? saved : 12;
  });
  ...
  const initApp = async () => {
    ...
    try {
      // Intentar carga de productos desde Supabase primero
      ...
      // Cargar categorías en segundo plano (no bloquea si falla)
      try {
        await loadCategories();
      } catch (catError) {
        console.warn("⚠️ Error cargando categorías, usando defaults:", catError);
      }
  ...
  const saveProductToSupabase = async (product) => {
    ...
    try {
      ...
      // CAMBIOS 2,3,5,7: Incluir nuevos campos
      const productData = {
        name: product.name,
        ...
        show_retail_price: product.show_retail_price || false,
        badges: product.badges || [],
        min_boxes: parseInt(product.min_boxes) || 1,
...
// const syncProductsWithSupabase = async (productsToSync) => { ... };
    ...
    const updatedCart = cart
      .map(cartItem => {
        ...
        return {
          ...
          show_retail_price: product.show_retail_price,
          badges: product.badges,
          min_boxes: product.min_boxes,
  ...
  // CAMBIO 7: Filtrar banners (no mostrar banners en la lista normal)
  result = result.filter(p => !p.is_banner);
  
  return result;
}, [products, cat, searchTerm, priceRange]);

// CAMBIO 8: Aplicar paginación
...
// CAMBIO 8: Resetear página cuando cambian filtros
useEffect(() => {
  setCurrentPage(1);
  ...
  // CAMBIO 1: Función para cargar categorías desde Supabase
  const loadCategories = async () => {
    const supabase = getSupabaseClient();
    if (!supabase) {
      console.warn('Configuración de Supabase no disponible, usando categorías por defecto');
      return;
    }
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Actualizar CAT_EMOJI y CAT_COLOR dinámicamente
        data.forEach(c => {
          if (c.emoji) CAT_EMOJI[c.name] = c.emoji;
          if (c.color) CAT_COLOR[c.name] = c.color;
        });
        
        console.log(`✅ ${categoryNames.length} categorías cargadas desde Supabase`);
      }
    } catch (error) {
  ...
  // CAMBIO 4: Función para calcular próxima fecha de entrega
  const getNextDeliveryDate = (zone) => {
    const now = new Date();
    const currentHour = now.getHours();
    const currentDay = now.getDay(); // 0 = Domingo, 1 = Lunes, etc.
    
    const zoneDays = {
      1: [1, 2], // Lunes y Martes
      2: [3, 4], // Miércoles y Jueves
      3: [5, 6]  // Viernes y Sábado
    };
    
    const zoneLocalities = {
      1: "Escobar, Tigre, San Fernando, Vicente López, Capital Federal",
      2: "Pilar, Malvinas Argentinas, San Miguel, Hurlingham, San Martín, Tres de Febrero",
      3: "José C. Paz, Moreno, Merlo, Morón, La Matanza y zonas aledañas"
    };
    
    const days = zoneDays[zone];
    if (!days) return null;
    
    // Si ya pasó la hora de corte, descartar el día de hoy
    let searchDay = currentDay;
    if (currentHour >= shippingCutoffHour) {
      searchDay = currentDay + 1;
      if (searchDay > 6) searchDay = 0; // Volver a Domingo
    }
    ...
    return {
      date: deliveryDate,
      formatted: `${dayName} ${deliveryDate.getDate()}/${deliveryDate.getMonth() + 1}`,
      localities: zoneLocalities[zone]
    };
  };
  ...
  const handleFormSubmit = async () => {
    ...
    console.log('🔍 handleFormSubmit - Inicio (simplificado):', {
      editing,
      name: form.name,
      price: form.price
    });
    
    // Crear producto simple solo con campos básicos
    const p = { 
      ...form, 
      id: editing ? form.id : `prod_${Date.now()}`, 
    ...
    try {
      if (editing) {
        ...
        if (result) {
          ...
          await saveProducts(updatedProducts, true, false);
          console.log('✏️ Producto actualizado en Supabase');
        ...
        if (newSupabaseId) {
          // Si se creó nuevo producto, actualizar con el UUID real
          const finalProduct = newSupabaseId !== true ? { ...p, id: newSupabaseId } : p;
          await saveProducts([...products, finalProduct], true, false);
          console.log('➕ Producto agregado a Supabase');
          showToast("➕ Producto agregado", "success");
        } else {
          throw new Error('Error creando en Supabase');
        }
      }
      
      // Limpiar formulario
      setForm({ id:"", category:"Frescos", name:"", description:"", price:"", bulkInfo:"", image_url:"", retail_price:"", show_retail_price:false, badges:[], min_boxes:1, is_banner:false, banner_title:"" });
      setEditing(false);
      setAdminTab("list");
      ...
      setTimeout(async () => {
        ...
        console.log('🔄 Productos recargados desde Supabase');
      }, 500);
  ...
  const startEdit = (p) => { 
    setForm({
      ...
      retail_price: p.retail_price ? p.retail_price.toString() : "",
      show_retail_price: p.show_retail_price || false,
      badges: p.badges || [],
      min_boxes: p.min_boxes || 1,
      is_banner: p.is_banner || false,
    ...
    <div style={{ minHeight:"100vh", background:"#F4F4F5", fontFamily:"'Poppins', sans-serif", position:"relative", maxWidth:"100%", overflowX:"hidden" }}>
      ...
      {view === "shop" ? (
        <>
          ...
          {/* CAMBIO 3: SHIPPING BANNER */}
          <div style={{ background:"linear-gradient(135deg, #C41E3A 0%, #8B0000 100%)", padding:"16px", textAlign:"center" }}>
            <div style={{ color:"white", fontSize:14, fontWeight:700, letterSpacing:0.5 }}>
              🚚 ENVÍO GRATIS (programados) — Coordina el día de entrega según tu zona
            </div>
          </div>
          ...
          <div style={{ background:"white", borderBottom:"1px solid #E5E7EB", padding:"16px", position:"sticky", top:106, zIndex:95 }}>
            <div style={{ maxWidth:1200, margin:"0 auto" }}>
              {/* CAMBIO 4: Zone Selector */}
              <ZoneSelector 
                selectedZone={selectedZone} 
                setSelectedZone={setSelectedZone} 
                getNextDeliveryDate={getNextDeliveryDate} 
                shippingCutoffHour={shippingCutoffHour} 
              />
          ...
          <div style={{ maxWidth:1200, margin:"0 auto", padding:"20px 12px 48px" }}>
            {/* CAMBIO 6: Kit Gastronómico */}
            {cat === "Todos" && <KitGastronomico />}
            
            {/* CAMBIO 7: Render banners first */}
            {products.filter(p => p.is_banner).map(banner => (
            ...
            {CATS.filter(c => c !== "Todos").map(c => {
              const prods = (paginatedProducts || [])?.filter(p => p && typeof p === 'object' && p?.category && p?.category === c);
        ...
        <AdminPanel 
            ...
            setShippingCutoffHour={setShippingCutoffHour}
            shippingCutoffHour={shippingCutoffHour}
  ...
  return (
    <div className="product-card" onClick={onOpen}>
      ...
      <div style={{ padding:"10px 11px 12px" }}>
        ...
        {/* CAMBIO 2: Mostrar precio mayorista y precio público */}
        <div style={{ marginBottom:6 }}>
          <div style={{ fontSize:9, color:"#6B7280", fontWeight:600, letterSpacing:0.5 }}>Precio Mayorista</div>
          <div style={{ fontWeight:900, fontSize:17, color:"#C41E3A" }}>{fmt(price || 0)}</div>
          {show_retail_price && retail_price > 0 && (
            <div style={{ fontSize:9, color:"#6B7280", fontWeight:600, letterSpacing:0.5, marginTop:2 }}>Precio Público</div>
          )}
          {show_retail_price && retail_price > 0 && (
            <div style={{ fontWeight:700, fontSize:13, color:"#9CA3AF", textDecoration: "line-through" }}>{fmt(retail_price)}</div>
          )}
        </div>
        
        {/* CAMBIO 5: Mostrar mínimo de cajas */}
        {min_boxes > 1 && (
          <div style={{ fontSize:10, color:"#059669", fontWeight:600, marginBottom:6 }}>
            Mínimo: {min_boxes} caja{min_boxes > 1 ? 's' : ''}
          </div>
        )}
    ...
    <div>
      ...
      <div style={{ padding:"20px 22px 24px" }}>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:10, color:"#6B7280", fontWeight:700, letterSpacing:1 }}>PRECIO MAYORISTA</div>
          <div style={{ fontWeight:900, fontSize:30, color:"#C41E3A", lineHeight:1 }}>{fmt(p?.price || 0)}</div>
          {p?.show_retail_price && p?.retail_price > 0 && (
            <>
              <div style={{ fontSize:10, color:"#6B7280", fontWeight:700, letterSpacing:1, marginTop:8 }}>PRECIO PÚBLICO</div>
              <div style={{ fontWeight:700, fontSize:18, color:"#9CA3AF", textDecoration: "line-through" }}>{fmt(p?.retail_price)}</div>
            </>
          )}
        </div>
        ...
        {/* CAMBIO 5: Mostrar mínimo de cajas */}
        {p?.min_boxes > 1 && (
          <div style={{ background:"#ECFDF5", borderRadius:10, padding:"10px 14px", marginBottom:16, border:"1px solid #A7F3D0" }}>
            <div style={{ fontSize:11, color:"#059669", fontWeight:700, letterSpacing:0.5 }}>MÍNIMO DE COMPRA</div>
            <div style={{ fontSize:14, fontWeight:700, color:"#059669" }}>{p.min_boxes} caja{p.min_boxes > 1 ? 's' : ''}</div>
          </div>
        )}
...
function CartDrawer({ cart, onRemove, onClose, total, onClear }) {
  ...
  // CAMBIO 5: Validar mínimo de cajas
  const validateMinBoxes = () => {
    const invalidItems = cart.filter(item => {
      const minBoxes = item.min_boxes || 1;
      return item.qty < minBoxes;
    });
    return invalidItems;
  };
  
  const invalidItems = validateMinBoxes();
    ...
    <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        ...
        {cart.length === 0 ? (
          ...
          <div key={item.id} style={{ display:"flex", gap:12, padding:"13px 0", borderBottom:"1px solid #F3F4F6", alignItems:"flex-start" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              {/* CAMBIO 5: Mostrar mínimo de cajas */}
              {item.min_boxes > 1 && (
                <div style={{ fontSize:10, color:item.qty < item.min_boxes ? "#DC2626" : "#059669", fontWeight:600, marginTop:2 }}>
                  {item.qty < item.min_boxes ? `⚠️ Mínimo: ${item.min_boxes} cajas` : `Mínimo: ${item.min_boxes} cajas ✓`}
                </div>
              )}
            </div>
      ...
      {cart.length > 0 && (
        <div style={{ padding:"16px 20px", borderTop:"1px solid #F3F4F6", background:"white" }}>
          {/* CAMBIO 5: Mostrar advertencias de mínimo de cajas */}
          {!canCheckout && (
            <div style={{ marginBottom:14, padding:"12px 14px", background:"#FEE2E2", borderRadius:12, border:"1px solid #FECACA" }}>
              <div style={{ fontWeight:700, color:"#991B1B", fontSize:13, marginBottom:8 }}>⚠️ Productos sin mínimo:</div>
              {invalidItems.map(item => (
                <div key={item.id} style={{ fontSize:12, color:"#991B1B", marginBottom:4 }}>
                  • {item.name}: requiere mínimo {item.min_boxes} cajas (tenés {item.qty})
                </div>
              ))}
            </div>
          )}
          ...
          <button onClick={sendWA} disabled={!canCheckout} style={{ width:"100%", background:!canCheckout ? "#9CA3AF" : "#25D366", color:"white", border:"none", borderRadius:12, padding:14, fontSize:15, fontWeight:700, cursor:!canCheckout ? "not-allowed" : "pointer", marginBottom:8, fontFamily:"'Poppins',sans-serif", opacity:!canCheckout ? 0.6 : 1 }}>
            📱 Enviar por WhatsApp
          </button>
...
// ═══════════════════════════════════════════════════════
// ZONE SELECTOR COMPONENT (CAMBIO 4)
// ═══════════════════════════════════════════════════════

function ZoneSelector({ selectedZone, setSelectedZone, getNextDeliveryDate, shippingCutoffHour }) {
  const zones = [
    { id: 1, name: "Zona 1", days: "Lunes y Martes" },
    { id: 2, name: "Zona 2", days: "Miércoles y Jueves" },
    { id: 3, name: "Zona 3", days: "Viernes y Sábado" }
  ];
  
  const deliveryInfo = selectedZone ? getNextDeliveryDate(selectedZone) : null;
  
  return (
    <div style={{ background:"white", borderRadius:16, padding:"20px", marginBottom:20, border:"1px solid #E5E7EB" }}>
      <h3 style={{ margin:"0 0 16px", fontSize:18, fontWeight:800, color:"#111" }}>📍 Coordiná el día de tu entrega</h3>
        ...
        {zones.map(zone => (
          <button
            ...
            style={{
              ...
              minWidth:120,
              padding:"12px 16px",
              background:selectedZone === zone.id ? "#C41E3A" : "white",
              color:selectedZone === zone.id ? "white" : "#374151",
              border:selectedZone === zone.id ? "none" : "1px solid #E5E7EB",
              borderRadius:10,
              fontSize:13,
              fontWeight:600,
              cursor:"pointer",
          ...
          >
            <div>{zone.name}</div>
            <div style={{ fontSize:11, opacity:0.8, marginTop:2 }}>{zone.days}</div>
          </button>
        ))}
      </div>
      ...
      {deliveryInfo && (
        <div style={{ background:"#F0FDF4", borderRadius:12, padding:"16px", border:"1px solid #BBF7D0" }}>
          <div style={{ fontSize:13, fontWeight:700, color:"#166534", marginBottom:8 }}>📦 Tu próxima entrega disponible: <span style={{ fontWeight:800 }}>{deliveryInfo.formatted}</span></div>
          <div style={{ fontSize:12, color:"#166534", marginBottom:8 }}>
            <strong>Zonas:</strong> {deliveryInfo.localities}
          </div>
          <div style={{ fontSize:11, color:"#166534", fontStyle:"italic" }}>
            ⏰ Hora de corte: {shippingCutoffHour}:00 hs
          </div>
        </div>
      )}
      
      <div style={{ marginTop:12, fontSize:11, color:"#6B7280", textAlign:"center" }}>
        Los pedidos realizados después de las {shippingCutoffHour}:00 hs se programan para el próximo día de reparto disponible
      </div>
    </div>
  );
}
...
// ═══════════════════════════════════════════════════════
// KIT GASTRONÓMICO COMPONENT (CAMBIO 6)
// ═══════════════════════════════════════════════════════

function KitGastronomico() {
  const kitProducts = [
    { name: "Kit gastronómico completo", price: 980000 },
    { name: "Mantenedor de alimentos calientes", price: 960000 },
    { name: "Base para horno", price: 180000 }
  ];
  ...
  return (
    <div style={{ 
      ...
      <div style={{ textAlign:"center", marginBottom:20 }}>
        ...
        <h3 style={{ margin:"0 0 8px", fontSize:20, fontWeight:800, letterSpacing:1 }}>Tenemos todos los productos para tu emprendimiento</h3>
        <div style={{ fontSize:14, fontWeight:600, color:"#FBD38D", letterSpacing:0.5 }}>Kit Gastronómico Profesional</div>
      </div>
      
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:16, marginBottom:20 }}>
        {kitProducts.map((product, idx) => (
          <div key={idx} style={{ background:"rgba(255,255,255,0.1)", borderRadius:12, padding:"16px", border:"1px solid rgba(255,255,255,0.2)" }}>
            <div style={{ fontSize:13, fontWeight:600, marginBottom:8 }}>{product.name}</div>
            <div style={{ fontSize:18, fontWeight:800, color:"#F5A623" }}>{fmt(product.price)}</div>
          </div>
        ))}
      </div>
      
      <div style={{ background:"rgba(255,255,255,0.1)", borderRadius:12, padding:"16px", marginBottom:16, border:"1px solid rgba(255,255,255,0.2)" }}>
        <div style={{ fontSize:13, fontWeight:600, marginBottom:4 }}>💳 Si tenés comercio podés abonar en 7 cuotas semanales de $309.000</div>
        <div style={{ fontSize:12, opacity:0.9 }}>Aceptamos todas las tarjetas</div>
      </div>
      
      <div style={{ background:"rgba(255,255,255,0.1)", borderRadius:12, padding:"12px", textAlign:"center", border:"1px solid rgba(255,255,255,0.2)" }}>
        <div style={{ fontSize:12, fontWeight:600 }}>🚚 Si ya compraste tu horno o calentador de alimentos, el envío es GRATIS con la compra de 3 cajas de empanadas</div>
      </div>
    </div>
  );
}
...
function Pagination({ currentPage, totalPages, setCurrentPage }) {
  ...
  return (
    <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:8, marginTop:24, padding:"16px 0" }}>
      <button
      ...
      {getPageNumbers().map((page, idx) => (
        page === '...' ? (
          <span key={idx} style={{ padding:"8px 12px", color:"#9CA3AF" }}>...</span>
        ) : (
          <button
            key={idx}
...
function AdminPanel({ products, form, setForm, editing, setEditing, adminTab, setAdminTab, onSubmit, onEdit, onDelete, onExcel, fileRef, supaUrl, supaKey, setSupaUrl, setSupaKey, onSync, syncing, onSaveSupa, onReset, onImageSelect, onClearImage, imagePreview, uploadingImage, onMigrate, onUpdateSinglePrice, onUpdateBulkPrices, onPreviewBulkPriceChanges, priceHistory, onMigrateImages, onSyncProducts, restorePoints, onCreateRestorePoint, onRestoreFromPoint, user, isMaster, onLogin, onLogout, email, password, setEmail, setPassword, authLoading, categories, setCategories, loadCategories, setShippingCutoffHour, shippingCutoffHour, setProductsPerPage, productsPerPage }) {
  const input = { width:"100%", padding:"10px 13px", borderRadius:9, border:"1px solid #E5E7EB", fontSize:14, fontFamily:"'Poppins',sans-serif", marginTop:5, outline:"none" };
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"16px 12px 48px" }}>
      ...
      {adminTab === "add" && (
        <div style={{ background:"white", borderRadius:16, padding:24, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            ...
            <div>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>PRECIO MAYORISTA *</label>
              <input type="number" value={form.price} onChange={e => setForm({...form,price:e.target.value})} style={input} placeholder="19725" />
            </div>
            {/* CAMBIO 2: Precio público */}
            <div>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>PRECIO PÚBLICO (opcional)</label>
              <input type="number" value={form.retail_price} onChange={e => setForm({...form,retail_price:e.target.value})} style={input} placeholder="25000" />
            </div>
            <div>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>MOSTRAR PRECIO PÚBLICO</label>
              <div style={{ marginTop:5, display:"flex", alignItems:"center", gap:8 }}>
                <input 
                  type="checkbox" 
                  checked={form.show_retail_price} 
                  onChange={e => setForm({...form,show_retail_price:e.target.checked})}
                  style={{ width:18, height:18, cursor:"pointer" }}
                />
                <span style={{ fontSize:13, color:"#374151" }}>Mostrar precio público en la tienda</span>
              </div>
            </div>
            {/* CAMBIO 5: Mínimo de cajas */}
            <div>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>MÍNIMO DE CAJAS (mayorista)</label>
              <input type="number" value={form.min_boxes} onChange={e => setForm({...form,min_boxes:e.target.value})} style={input} placeholder="1" min="1" />
            </div>
            {/* CAMBIO 3: Badges */}
            <div style={{ gridColumn:"1/-1" }}>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>BADGES (etiquetas)</label>
              <div style={{ marginTop:5, display:"flex", flexWrap:"wrap", gap:8 }}>
                {["Oferta", "Nuevo", "Mayorista"].map(badge => (
                  <label key={badge} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer" }}>
                    <input 
                      type="checkbox" 
                    ...
                    />
                    <span style={{ fontSize:13, color:"#374151" }}>{badge}</span>
                  </label>
                ))}
              </div>
            </div>
            {/* CAMBIO 7: Banner separador */}
            <div>
              <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>ES BANNER SEPARADOR</label>
              <div style={{ marginTop:5, display:"flex", alignItems:"center", gap:8 }}>
                <input 
                  type="checkbox" 
                  ...
                  style={{ width:18, height:18, cursor:"pointer" }}
                />
                <span style={{ fontSize:13, color:"#374151" }}>Marcar como banner separador (no es un producto real)</span>
              </div>
            </div>
            {form.is_banner && (
              <div style={{ gridColumn:"1/-1" }}>
                <label style={{ fontSize:12, fontWeight:700, color:"#6B7280", letterSpacing:0.5 }}>TÍTULO DEL BANNER</label>
                <input value={form.banner_title} onChange={e => setForm({...form,banner_title:e.target.value})} style={input} placeholder="Ej: 🔥 Alquiler de Horno" />
              </div>
            )}
            <div style={{ gridColumn:"1/-1" }}>
      ...
      {/* CAMBIO 1: TAB: CATEGORIES */}
      {adminTab === "categories" && (
        <div style={{ background:"white", borderRadius:16, padding:24 }}>
          <h3 style={{ margin:"0 0 6px", fontWeight:800 }}>📁 Gestión de Categorías</h3>
          <p style={{ color:"#6B7280", fontSize:14, marginBottom:20 }}>Administra las categorías de productos desde Supabase.</p>
          
          <div style={{ marginBottom:24, padding:16, background:"#F9FAFB", borderRadius:12, border:"1px solid #E5E7EB" }}>
            <div style={{ fontWeight:700, color:"#374151", marginBottom:12, fontSize:14 }}>Crear nueva categoría</div>
              ...
              <button 
                onClick={async () => {
                  ...
                  try {
                    const { error } = await supabase
                      .from('categories')
                      .insert({ name, emoji, color: color || '#C41E3A' });
                    if (error) throw error;
                    await loadCategories();
          ...
          <div style={{ fontWeight:700, color:"#374151", marginBottom:12, fontSize:14 }}>Categorías existentes</div>
          <div style={{ display:"grid", gap:8 }}>
            {categories.map(cat => (
              <div key={cat} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px", background:"white", border:"1px solid #E5E7EB", borderRadius:8 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <button 
                  ...
                  style={{ padding:"6px 12px", background:"#DC2626", color:"white", border:"none", borderRadius:6, cursor:"pointer", fontSize:12 }}
                >
                  Eliminar
                </button>
              </div>
      ...
      {/* CAMBIO 4: TAB: SHIPPING ZONES */}
      {adminTab === "shipping" && (
        <div style={{ background:"white", borderRadius:16, padding:24 }}>
          <h3 style={{ margin:"0 0 6px", fontWeight:800 }}>📍 Configuración de Envíos</h3>
          <p style={{ color:"#6B7280", fontSize:14, marginBottom:20 }}>Configura las zonas de envío y horarios de entrega.</p>
          
          <div style={{ marginBottom:24, padding:16, background:"#F9FAFB", borderRadius:12, border:"1px solid #E5E7EB" }}>
            <div style={{ fontWeight:700, color:"#374151", marginBottom:12, fontSize:14 }}>Hora de corte para pedidos</div>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (val >= 0 && val <= 23) {
                    setShippingCutoffHour(val);
                    localStorage.setItem("shipping_cutoff_hour", val);
                  }
                }}
                min="0"
                max="23"
                style={{ padding:"10px", borderRadius:8, border:"1px solid #E5E7EB", fontSize:14, width:80 }}
              />
              <span style={{ fontSize:14, color:"#6B7280" }}>:00 hs (pedidos después de esta hora van al próximo día de reparto)</span>
            </div>
          </div>
          
          <div style={{ fontWeight:700, color:"#374151", marginBottom:12, fontSize:14 }}>Zonas de reparto</div>
          <div style={{ display:"grid", gap:16 }}>
            <div style={{ padding:16, background:"white", border:"1px solid #E5E7EB", borderRadius:8 }}>
              <div style={{ fontWeight:600, marginBottom:8, color:"#C41E3A" }}>Zona 1 - Lunes y Martes</div>
              <div style={{ fontSize:13, color:"#6B7280" }}>Escobar, Tigre, San Fernando, Vicente López, Capital Federal</div>
            </div>
            <div style={{ padding:16, background:"white", border:"1px solid #E5E7EB", borderRadius:8 }}>
              <div style={{ fontWeight:600, marginBottom:8, color:"#C41E3A" }}>Zona 2 - Miércoles y Jueves</div>
              <div style={{ fontSize:13, color:"#6B7280" }}>Pilar, Malvinas Argentinas, San Miguel, Hurlingham, San Martín, Tres de Febrero</div>
            </div>
            <div style={{ padding:16, background:"white", border:"1px solid #E5E7EB", borderRadius:8 }}>
              <div style={{ fontWeight:600, marginBottom:8, color:"#C41E3A" }}>Zona 3 - Viernes y Sábado</div>
              <div style={{ fontSize:13, color:"#6B7280" }}>José C. Paz, Moreno, Merlo, Morón, La Matanza y zonas aledañas</div>
            </div>
          </div>
          
          <div style={{ marginTop:24, padding:16, background:"#F9FAFB", borderRadius:12, border:"1px solid #E5E7EB" }}>
            <div style={{ fontWeight:700, color:"#374151", marginBottom:12, fontSize:14 }}>Productos por página</div>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <input 
                type="number" 
                ...
                style={{ padding:"10px", borderRadius:8, border:"1px solid #E5E7EB", fontSize:14, width:80 }}
              />
              <span style={{ fontSize:14, color:"#6B7280" }}>productos por página en la tienda</span>
            </div>
          </div>
        </div>
      )}
